...
public class StreamHelper {
  ...
  /**
   * Shorthand for filtering based on the negation of a predicate.
   *
   * @param s         the stream to filter
   * @param exclusion the predicate that must NOT hold on items in the stream
   * @param <T>       type of elements in the stream
   * @return a stream corresponding to the filtering of s by the negation of exclusion
   */
  public static <T> Stream<T> exclude(Stream<T> s, Predicate<T> exclusion) {
    return s.filter(x -> !exclusion.test(x));
  }
